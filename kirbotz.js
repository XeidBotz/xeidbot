...
🛒 𝗦𝗲𝗹𝗹 𝗜𝗻𝘃𝗲𝗻𝘁𝗼𝗿𝘆
 ...
 × ${prefix}sellemas
 × ${prefix}selldiamond`
        kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: thumbrpg }, templateButtons: button, footer: `Rpg Game By XeidBot`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
「 PROFILE 」
 ...
 • Xp :  ${getLevelingXp(sender)}/${reqXp}
「 RPG GAME 」`
        kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgjoinrpg }, templateButtons: button, footer: `Rpg Game By XeidBot`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
addDm(sender, d)
        ...
        kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgslime }, templateButtons: button, footer: `Rpg Game By XeidBot`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: imggoblin }, templateButtons: button, footer: `Rpg Game By XeidBot`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgdevil }, templateButtons: button, footer: `Rpg Game By XeidBot`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgbehemoth }, templateButtons: button, footer: `Rpg Game By XeidBot`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgdemon }, templateButtons: button, footer: `Rpg Game By XeidBot`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgdemonking }, templateButtons: button, footer: `Rpg Game By XeidBot`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
var button = [
{"urlButton": {"displayText": "MyOwner🔥","url": `https://wa.me/6283842873526`}},
{"quickReplyButton": {"displayText": "Sewabot","id": 'sewabot'}},
{"quickReplyButton": {"displayText": "Donasi","id": 'donasi'}},
{"quickReplyButton": {"displayText": "Status Bot XeidBot","id": 'ping'}}
		]
kirbotz.sendMessage(from, { caption: teks, location: { jpegThumbnail: thumb }, templateButtons: button, footer: `Created By XeidBot`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
switch (command) {
case 'listpremium':
if (!m.key.fromMe && !isCreator) return replyNya(mess.wait)
teks = '*List Premium*\n\n'
...
replyNya(mess.wait)
nyenye = fs.readFileSync('./Lib/cecan.js')
...
hydratedContentText: `Jangan Tegang kak`,
hydratedFooterText: 'Created By XeidBot',
hydratedButtons: [{
urlButton: {
displayText: 'MyGithub',
url: 'https://github.com/XeidBot'
}
},
{
urlButton: {
displayText: 'MyOwner',
url: 'https://wa.me/6283842873526'
}
},
...
♕ FXdev @${soky.split("@")[0]}
♕ Fardan @${soki.split("@")[0]}`
kirbotz.sendMessage(m.chat, { image: thumb, caption: teksits, contextInfo: {"mentionedJid": [sokl, sokr, sokt, soky, soki]} }, { quoted: m })
break
case 'quest':
if (!isPrem) return replyPremium(mess.premium)
let sectionnya= [
...
kirbotz.sendList(m.chat, `Haii *${pushname}*`, `Silahkan Pilih Di Bawah`, "CLICK", "Rpg Game By XeidBot ", sectionnya, { quoted: m})
break
case 'slime':
case 'killslime':
if (!isPrem) return replyPremium(mess.premium)
...
case 'bye': {
if (!m.isGroup) return
await replyTemplate(`Baru Aja Masok Langsung Keluar🗿☕`)
}
break
case 'groupxeidbot': {
...
await kirbotz.send5ButGif(from, tampilan(ya, pushname, sender, isPrem, role, elit, namaownernya, namabotnya, runtime, tanggal, wib, wit, wita) , `Created By XeidBot`, [
{"urlButton": {"displayText": "MyOwner🔥","url": `https://wa.me/6283842873526`}},
...
│ ${ya}${prefix}groupxeidbot${ya}
│ ${ya}${prefix}rules${ya}
│ ${ya}${prefix}donasi${ya}
│ ${ya}${prefix}sewabot${ya}
│ ${ya}${prefix}scriptbot${ya}
...
│ ${ya}${prefix}cerpen${ya}
│ ${ya}${prefix}cerpen2${ya}
│ ${ya}${prefix}ceritahoror2${ya}
...
kirbotz.send5Loc(m.chat, `On Kak Silahkan Pencet Button Di Bawah\nJika Button Tidak Kelihatan Ketik ${prefix}menu`, `Created By XeidBot`, global.thumb, btn)
}
break
case 'y':{
replyNya(tampilan(ya, pushname, sender, isPrem, role, elit, namaownernya, namabotnya, runtime, tanggal, wib, wit, wita))
}
break
case 'userkirbotz':{
...
kirbotz.send5ButImg(i, junn, `Created By XeidBot`, buffer, butoon)
} else if (/video/.test(mime)) {
let junn = `*_BROADCAST VIDIO_*${text ? '\n\n' + text : ''}`
kirbotz.send5Vid(i, junn, `Created By XeidBot`, buffer, butoon)
...
case 'namapeople':{
if (!isPrem) return replyPremium(mess.premium)
get_result = await fetchJson(`https://api.lolhuman.xyz/api/random/people?apikey=${lolkey}`)
get_result = get_result.result
ini_txt = `Nama : ${get_result.name.title} ${get_result.name.first} ${get_result.name.last}\n`
ini_txt += `Gender : ${get_result.gender}\n`
ini_txt += `Location Number : ${get_result.location.street.number}\n`
ini_txt += `Location Name : ${get_result.location.street.name}\n`
ini_txt += `Location City : ${get_result.location.city}\n`
ini_txt += `Location State : ${get_result.location.state}\n`
ini_txt += `Location Country : ${get_result.location.country}\n`
ini_txt += `Location PostCode : ${get_result.location.postcode}\n`
ini_txt += `Location Latitude : ${get_result.location.coordinates.latitude}\n`
ini_txt += `Location Longitude : ${get_result.location.coordinates.longitude}\n`
ini_txt += `Location Timeone : ${get_result.location.timezone.offset}\n`
ini_txt += `Deskripsi : ${get_result.location.timezone.description}\n`
let buttons = [
{buttonId: `${command}`, buttonText: {displayText: 'Next'}, type: 1}
]
let buttonMessage = {
...
if (!text) throw oh
replyNya(mess.wait)
get_result = await fetchJson(`https://leyscoders-api.herokuapp.com/api/mediafire?url=${text}&apikey=IkyOgiwara`)
mnya = await getBuffer(get_result.result.download)
kirbotz.sendMessage(m.chat, {document: mnya, mimetype: 'application/zip', fileName: `Download.zip`}, {quoted:m}).catch ((err) => reply(oh))
}
break
case 'iqra': {
if (!isPrem) return replyPremium(mess.premium)
...
if (!isPrem) return replyPremium(mess.premium)
replyNya(mess.wait)
get_result = await fetchJson(`https://leyscoders-api.herokuapp.com/api/cersex?apikey=IkyOgiwara`)
buff = await getBuffer(get_result.gambar)
let message = await prepareWAMessageMedia({ image: buff }, { upload: kirbotz.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
imageMessage: message.imageMessage,
hydratedContentText: `${get_result.result}`,
hydratedFooterText: 'Created By XeidBot',
hydratedButtons: [{
urlButton: {
displayText: 'MyGithub',
...
if (!isPrem) return replyPremium(mess.premium)
replyNya(mess.wait)
get_result = await fetchJson(`https://leyscoders-api.herokuapp.com/api/cerita-horor?apikey=IkyOgiwara`)
get_result = get_result.result
ini_txt = `Title : ${get_result.title}\n`
...
if (!isPrem) return replyPremium(mess.premium)
replyNya(mess.wait)
get_result = await fetchJson(`https://leyscoders-api.herokuapp.com/api/cerpen?apikey=IkyOgiwara`)
get_result = get_result.result
ini_txt = `Title : ${get_result.title}\n`
ini_txt += `Pengarang : ${get_result.pengarang}\n`
ini_txt += `Kategori : ${get_result.kategori}\n`
ini_txt += `Story : ${get_result.story}\n`
replyNya(ini_txt)
}
break
case 'ceritahoror':{
...
var button = [
        ...
        kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: cing }, templateButtons: button, footer: `Rpg Game By XeidBot`, mentions: [sender] })
...
var button = [
        ...
        var hg = `「 DEATH 」\n\n • Tempat  ${ad}\n • MONEY : $${money}\n • EXP : ${adven}Xp`
        kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: hasm }, templateButtons: button, footer: `Rpg Game By XeidBot`, mentions: [sender] })
        .catch ((err) => reply(mess.error))
}, 7000)
setTimeout( () => {
...
List Payment
QRIS ALL : Scan Qr Di Atas
OVO : 0838-4287-3526
DANA : 0838-4287-3526

Note : MAU SEWABOT APA MAU LIAT DOANG CUG
...
let jawab = `Ciee Jadian💖 Jangan Lupa Pajak Jadiannya Yaa🐤
...
case 'xnxxsearch':
if (!isPrem) return replyPremium(mess.premium)
if (!text) return replyNya('Nyari Apa?')
pepex = await fetchJson(`https://bx-hunter.herokuapp.com/api/xvideosearch?query=${text}&apikey=FuckBitch`)
replyNya(mess.wait)
pepex = pepex.result
ini_txt = ""
for (var x of pepex) {
...
anu = `${ini_txt}───────────────\n\n┌ ◪ *DOWNLOAD*
└ ${prefix}xnxxvideo [link xvid] = Video`
kirbotz.sendMessage(from, { text: anu }, { quoted: m })
break
case 'xnxxvideo':
if (!isPrem) return replyPremium(mess.premium)
if (!text) return replyNya('Linknya?')
x = await fetchJson(`https://bx-hunter.herokuapp.com/api/xvideodetail?url=${text}&apikey=FuckBitch`)
replyNya(mess.wait)
replyNya(`Download Sendiri Nih
Link Nya ${x.result.files.low}`)
break
case 'igvideo':{
if (!isPrem) return replyPremium(mess.premium)
...
if (!isPrem) return replyPremium(mess.premium)
replyNya(mess.wait)
let eek = await fetchJson(`https://raw.githubusercontent.com/jepribarus/JB-Api/main/nsfw/${command}.json`)
let random = eek[Math.floor(Math.random() * eek.length)]
kirbotz.sendMessage(m.chat, { image: { url: random }, caption: `Nih Kak` }, { quoted: m })
}
break
